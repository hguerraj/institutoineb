 ...
 *      Secure::logout();
 *
 *      --- Page requires authentication AND FULL READ rights on specific table(*) :
 *      Secure::lock($table);
 *
 *      --- Page requires authentication AND RESTRICTED READ rights on specific table(*) :
...
class Secure
{
    ...
    public static function testUser()
    {
        ...
        if (isset($_POST['login-form'])) { // if user has posted the login form
            ...
            $columns = array(
                ...
                USERS_TABLE . '.active',
                USERS_TABLE . '_profiles.profile_name AS profiles_name'
            );
    ...
    /**
     * @param string $table
     * @return array An array with the restriction SQL
     */
    public static function getRestrictionQuery($table)
    {
        ...
        $restriction_query[] = str_replace($find, $replace, $UsersRights->tables[$table]['constraint_query']);

        if (is_null($UsersRights->tables[$table]['constraint_query'])) {
            $UsersRights->tables[$table]['constraint_query'] = '';
        }
