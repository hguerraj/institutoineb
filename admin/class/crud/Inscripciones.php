...
class Inscripciones extends Elements
{
    ...
    public $ID = array();
    public $Alumno_ID = array();
    public $Grado_ID = array();
    public $fecha_inscripcion = array();
    public $monto_inscripcion = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN alumnos ON inscripciones.Alumno_ID=alumnos.ID LEFT JOIN grados ON inscripciones.Grado_ID=grados.ID';
        $columns = 'inscripciones.ID, alumnos.ID AS alu_ID, grados.ID AS gra_ID, inscripciones.fecha_inscripcion, inscripciones.monto_inscripcion, inscripciones.Estado_Pago';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'inscripciones' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->ID[] = $row->ID;
                $this->Alumno_ID[] = $row->alu_ID;
                $this->Grado_ID[] = $row->gra_ID;
                $this->fecha_inscripcion[] = $row->fecha_inscripcion;
                $this->monto_inscripcion[] = $row->monto_inscripcion;
