...
class Profesores extends Elements
{
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN grados ON profesores.Grado_ID=grados.ID';
        $columns = 'profesores.ID, profesores.Nombre, profesores.Fecha_Nacimiento, profesores.direccion, profesores.telefono, profesores.Email, profesores.Asignaturas, profesores.Horario, grados.ID AS gra_ID, profesores.Usuario_ID';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'profesores' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->Asignaturas[] = $row->Asignaturas;
