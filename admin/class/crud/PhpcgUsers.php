...
class PhpcgUsers extends Elements
{
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN phpcg_users_profiles ON phpcg_users.profiles_id=phpcg_users_profiles.id';
        $columns = 'phpcg_users.id, phpcg_users_profiles.profile_name AS php_use_pro_pro_nam, phpcg_users.name, phpcg_users.firstname, phpcg_users.address, phpcg_users.city, phpcg_users.zip_code, phpcg_users.email, phpcg_users.phone, phpcg_users.mobile_phone, phpcg_users.password, phpcg_users.active';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'phpcg_users' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->id[] = $row->id;
                $this->profiles_id[] = $row->php_use_pro_pro_nam;
                $this->name[] = $row->name;
