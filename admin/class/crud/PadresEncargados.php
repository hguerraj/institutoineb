...
class PadresEncargados extends Elements
{
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN alumnos ON padres_encargados.Alumno_ID=alumnos.ID';
        $columns = 'padres_encargados.ID, padres_encargados.Nombre, padres_encargados.telefono, padres_encargados.Email, padres_encargados.direccion, padres_encargados.relacion_alumno, alumnos.ID AS alu_ID';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'padres_encargados' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->relacion_alumno[] = $row->relacion_alumno;
