...
class Notas extends Elements
{
    ...
    public $Unidad_ID = array();
    public $calificacion = array();
    public $Periodo_Academico = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN alumnos ON notas.Alumno_ID=alumnos.ID LEFT JOIN unidades ON notas.Unidad_ID=unidades.ID';
        $columns = 'notas.ID, alumnos.ID AS alu_ID, unidades.ID AS uni_ID, notas.calificacion, notas.Periodo_Academico';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'notas' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
