...
class Cursos extends Elements
{
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN grados ON cursos.Grado_ID=grados.ID';
        $columns = 'cursos.ID, cursos.Nombre_Curso, cursos.descripcion, grados.ID AS gra_ID';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'cursos' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
