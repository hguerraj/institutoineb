...
for ($i=0; $i < $generator->columns_count; $i++) {
    ...
    // skip primary keys
    if (!in_array($column_name, $generator->auto_increment_keys)) {
        if ($field_type == 'color' || $field_type == 'email' || $field_type == 'hidden' || $field_type == 'text' || $field_type == 'textarea' || $field_type == 'url') {
?>
...
?>
        ...
        try {
            ...
            if (DEMO !== true && !$db->update('<?php echo $generator->table; ?>', $values, $where, DEBUG_DB_QUERIES)) {
                $error = $db->error();
...
if ($show_external === true) {
    foreach ($active_ext_cols as $key => $ext_col) {
        ...
        // many to many
        // the posted records will be added / removed from the intermediate table.
...
?>
                ...
                // Array with <?php echo $intermediate_table; ?>.<?php echo $relation_origin_column; ?>

                $<?php echo $intermediate_table ?>_records_to_delete = array();

                $from = '<?php echo $intermediate_table; ?>';
                $columns = array('<?php echo $relation_target_column; ?>');
                $where = array('<?php echo $relation_origin_column; ?>' => $_SESSION['<?php echo $generator->table; ?>_editable_primary_keys']['<?php echo $origin_table . '.' . $origin_column; ?>']);
                ...
                if (!empty($db_count)) {
                    while ($row = $db->fetch()) {
                        $<?php echo $intermediate_table ?>_current_records[] = $row-><?php echo $relation_target_column; ?>;
                    }
                }
                ...
                foreach ($<?php echo $intermediate_table ?>_current_records as $<?php echo $target_table ?>_value) {
                    if (!in_array($<?php echo $target_table ?>_value, $_POST['ext_<?php echo $target_table; ?>'])) {
                        $<?php echo $intermediate_table ?>_records_to_delete[] = $<?php echo $target_table ?>_value;
                    }
                }
...
?>
                    $values['<?php echo $intermediate_table_auto_incremented_pk_column; ?>'] = '';
<?php
        }
?>
                    $values['<?php echo $relation_origin_column; ?>'] = $_SESSION['<?php echo $generator->table; ?>_editable_primary_keys']['<?php echo $origin_table . '.' . $origin_column; ?>'];
                ...
                foreach ($<?php echo $intermediate_table ?>_records_to_delete as $<?php echo $relation_target_column; ?>_value) {
                    $where = array();
                    $where['<?php echo $relation_origin_column; ?>'] = $_SESSION['<?php echo $generator->table; ?>_editable_primary_keys']['<?php echo $origin_table . '.' . $origin_column; ?>'];
                    $where['<?php echo $relation_target_column; ?>'] = $<?php echo $relation_target_column; ?>_value;
                    if (DEMO !== true && !$db->delete('<?php echo $intermediate_table; ?>', $where, DEBUG_DB_QUERIES)) {
                        $error = $db->error();
                        throw new \Exception($error);
                    }
                }
...
if (!empty($join_query)) {
?>
    $from = '<?php echo $generator->table; ?> <?php echo $join_query; ?>';
    $columns = '<?php echo $fields; ?>';
<?php
} else {
?>
...
?>
    ...
    if (ADMIN_LOCKED === true && Secure::canUpdateRestricted('<?php echo $generator->table; ?>')) {
        $where = array_merge($where, Secure::getRestrictionQuery('<?php echo $generator->table; ?>'));
    }
...
for ($i=0; $i < $generator->columns_count; $i++) {
    ...
    if ($field_type == 'datetime') {
        if (intval($generator->columns['special3'][$i]) < 1) {
?>
    $<?php echo $column_name; ?>_ts = strtotime($row-><?php echo $column_name; ?>);
    // if timestamp is valid and > '1970-01-01'
    if (Utils::isValidTimeStamp($<?php echo $column_name; ?>_ts) && $<?php echo $column_name; ?>_ts > -62169984000) {
        ...
        $_SESSION['<?php echo $form_id; ?>']['<?php echo $column_name; ?>'] = null;
        $_SESSION['<?php echo $form_id; ?>']['<?php echo $column_name; ?>-time'] = null;
    }
<?php
        } else {
?>
...
?>
    ...
    // if timestamp is valid and > '1970-01-01'
    if (Utils::isValidTimeStamp($<?php echo $column_name; ?>_ts) && $<?php echo $column_name; ?>_ts > -62169984000) {
        $_SESSION['<?php echo $form_id; ?>']['<?php echo $column_name; ?>'] = date('Y', $<?php echo $column_name; ?>_ts);
    } else {
        $_SESSION['<?php echo $form_id; ?>']['<?php echo $column_name; ?>'] = null;
    }
<?php
...
?>
    $_SESSION['<?php echo $form_id; ?>']['<?php echo $column_name; ?>'] = date('Y');
<?php
        }
...
$form->setAction(ADMIN_URL . '<?php echo $generator->item ?>/edit/' . $pk_url_params);
$form->startFieldset();
<?php
...
for ($i=0; $i < $generator->columns_count; $i++) {
    // if (!in_array($generator->columns['name'][$i], $generator->auto_increment_keys)) {
        ...
        // attributes
        $attr = [];
        if ($required && $field_type != 'password') {
            $attr[] = 'required';
        }
...
<?php
        ...
        if (($i == 0 && $flex == 'start') || ($i > 0 && isset($flex_option[$i - 1]) && $flex != $flex_option[$i - 1])) {
?>
$options = array(
...
<?php
        ...
        // layout
        if ($i < 1 || !isset($field_width[$i - 1]) || $width != $field_width[$i - 1] || in_array($generator->columns['name'][$i - 1], $generator->auto_increment_keys)) {
?>
