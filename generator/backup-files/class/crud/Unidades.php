...
class Unidades extends Elements
{
    ...
    public $ID = array();
    public $Curso_ID = array();
    public $Nombre_Unidad = array();
    public $descripcion = array();
    public $numero_unidad = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $this->join_query = ' LEFT JOIN cursos ON unidades.Curso_ID=cursos.ID';
        $columns = 'unidades.ID, cursos.ID AS cur_ID, unidades.Nombre_Unidad, unidades.descripcion, unidades.numero_unidad';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'unidades' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->ID[] = $row->ID;
                $this->Curso_ID[] = $row->cur_ID;
                $this->Nombre_Unidad[] = $row->Nombre_Unidad;
                $this->descripcion[] = $row->descripcion;
                $this->numero_unidad[] = $row->numero_unidad;
            }
        }
