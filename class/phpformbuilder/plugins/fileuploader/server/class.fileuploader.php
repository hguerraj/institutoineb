...
class FileUploader
{
    ...
    public function __construct($name, $options = null)
    {
        $this->default_options['move_uploaded_file'] = function ($tmp, $dest, $item) {
            if (strpos($_SERVER['HTTP_REFERER'], 'drag-n-drop-form-builder/ajax/preview.php') === false) {
                return move_uploaded_file($tmp, $dest);
            } else {
                // simulate the upload if we're in the drag & drop preview
                return true;
            }
        };
    ...
    private function generateFilename($conf, $item, $skip_replace_check = false)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                $string = str_replace("{random}", $randomString, $string);
        ...
        $string = self::filterFilename($string);

        // generate another filename if a file with the same name already exists
        // only when replace options is false
        // file_put_contents($this->options['uploadDir'] . 'debug.log', 'replace = ' . $this->options['replace'] . "\n", FILE_APPEND);
        if (!$this->options['replace'] && !$skip_replace_check) {
            $title = $item['title'];
            // file_put_contents($this->options['uploadDir'] . 'debug.log', 'Check ' . $title . "\n", FILE_APPEND);
            $i = 1;

            while (file_exists($this->options['uploadDir'] . $string)) {
                $item['title'] = $title . "-{$i}";
                // file_put_contents($this->options['uploadDir'] . 'debug.log', 'New title = ' . $item['title'] . "\n", FILE_APPEND);
                $conf[0] = $type == "auto" || $type == "name" || strpos($string, "{random}") !== false ? $type : $type  . "-{$i}";
                $string = $this->generateFileName($conf, $item, true);
                $i++;
            }
            // file_put_contents($this->options['uploadDir'] . 'debug.log', "---\n\n", FILE_APPEND);
        }
    ...
    public static function filterFilename($filename)
    {
        ...
        $new_name = preg_replace('`[ &~"#{( \'\[|\\^@)\]=}$¤*µ%,;:!?/§]+`', '-', $new_name);
        while (preg_match('`--`', $new_name)) {
     ...
     * @return {String}
     */
    public static function mimeContentType($file, $nativeFunction = false)
    {
